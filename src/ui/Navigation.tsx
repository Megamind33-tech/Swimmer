        ...
        {navItems.map((item) => {
          ...
          return (
            <button key={item.id} onClick={() => setActiveTab(item.id)}
              aria-label={item.label}
              title={item.label}
              className={`flex flex-col items-center justify-center w-full h-full transition-all btn-mech relative focus-visible:ring-2 focus-visible:ring-[#D4A843] focus-visible:outline-none focus-visible:-outline-offset-2 ${
