          ...
          <div key={section.label} className="space-y-2">
            ...
            <div className="grid gap-2">
              {section.items.map((setting) => (
                <button key={setting.key} onClick={() => handleToggle(setting.key)} role="switch" aria-checked={setting.state}
                  ...
                  <div aria-hidden="true" className={`w-10 h-5 rounded-full p-0.5 transition-colors shrink-0 ml-3 ${setting.state ? `bg-[${section.color}]` : 'bg-[#1E3A57]'}`}
                    style={{ backgroundColor: setting.state ? section.color : '#1E3A57' }}>
                    <div aria-hidden="true" className={`w-4 h-4 rounded-full bg-[#0a192f] transition-transform ${setting.state ? 'translate-x-5' : 'translate-x-0'}`} />
                  </div>
                </button>
              ))}
            </div>
