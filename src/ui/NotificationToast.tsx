    ...
    <div className="fixed top-[62px] right-4 z-[300] flex flex-col gap-2 pointer-events-auto max-w-[320px]">
      {state.notifications.slice(-3).map((n) => (
        ...
        >
          ...
          <button onClick={() => dispatch({ type: 'DISMISS_NOTIFICATION', id: n.id })} aria-label="Dismiss notification" title="Dismiss" className="opacity-60 hover:opacity-100 btn-mech shrink-0 focus-visible:ring-2 focus-visible:ring-[#D4A843] focus-visible:outline-none">
            <X size={14} />
          </button>
        </div>
      ))}
