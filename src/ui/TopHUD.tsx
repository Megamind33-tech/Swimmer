  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-[48px] landscape:h-[44px] bg-[#0a192f]/95 border-b border-[#1E3A57] backdrop-blur-md flex items-center px-3 gap-3 z-100">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <button onClick={() => openOverlay('profile')} aria-label="User Profile" title="User Profile" className="w-[36px] h-[36px] border-2 border-[#18C8F0] p-0.5 cursor-pointer rounded-full overflow-hidden btn-mech shrink-0">
          {avatarImg ? (
            <img src={avatarImg} alt="Player" className="w-full h-full object-cover rounded-full" />
          ) : (
            <div className="w-full h-full bg-[#112240] flex items-center justify-center rounded-full"><User size={16} aria-hidden="true" className="text-[#18C8F0]"/></div>
          )}
        </button>
      </div>
    </header>
